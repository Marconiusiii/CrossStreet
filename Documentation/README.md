...
`statusText` is the main piece of screen text for results and errors. The existing text stays visible while a new command runs. Each operation starts the gold outline directly instead of waiting for SwiftUI to observe a loading-state change, so even an operation that finishes almost immediately begins the fade-in. The outline stays visible for at least 1.25 seconds. If the command takes longer, the outline remains until it finishes. Its bottom edge is inset by two points so the adjacent Nearest button border cannot overlap it and make it look thinner than the other three sides. If an operation is still running after two seconds, a large native circular loading indicator appears in a permanently reserved area at the bottom center of Current Info. Its position and clearance from the border remain stable when the text changes or grows with Dynamic Type. Its moving arc uses the app's gold accent, communicates activity without implying measurable progress, and does not become a separate VoiceOver element. At the same two-second point, an intersection lookup starts its loading sound and sends a low-priority VoiceOver status announcement saying that Intersector is still finding the intersection. If the lookup continues for another eight seconds, a second low-priority status announcement explains that it is still working.

When the result or error arrives, the loading indicator disappears and the new text replaces the old text with a short fade. The gold outline fades out at that point if it has already been visible for 1.25 seconds; otherwise it remains until that minimum time has passed so a quick command does not create a flash. Without Reduce Motion, the incoming text also moves upward by four points. With Reduce Motion, the transition uses only the fade. The completed result uses a high-priority VoiceOver announcement.
...
3. Shows the gold Current Info outline immediately without replacing the existing Current Info text.
...
7. Replaces `statusText` using the fade as the delayed loading indicator disappears and the outline completes its minimum presentation.
8. Sends the completed result or error through `VoiceOverAnnouncer.reportUpdated(_:)`.
9. Clears loading state and hides the delayed loading indicator.
...
While scanning, each heading update is checked against nearby intersections. When the phone points close enough to an intersection, the controller builds a scan report, updates the screen, sends haptics if enabled, and announces the result. Each identified intersection also starts the gold Current Info border immediately and keeps it visible for at least 1.25 seconds. Because the result has already been identified, this brief emphasis does not start the delayed loading throbber.
